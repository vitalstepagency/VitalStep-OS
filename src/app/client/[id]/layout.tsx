...
export default function ClientLayout({ children }: ClientLayoutProps) {
  ...
  useEffect(() => {
    ...
    const publicMetadata = user?.publicMetadata as
      | { role?: string | string[]; clientId?: string }
      | undefined;
    ...
    console.log("🔍 CLIENT LAYOUT ROLE CHECK:", {
      roles,
      isAdmin,
      isClient,
      hasValidRole,
    ...
    if (!hasValidRole && !isAssigningRole) {
      console.log(
        "❌ CLIENT LAYOUT: No valid role found, auto-assigning client role"
      );
      ...
      fetch("/api/user/self-assign", {
        ...
        .then((data) => {
          if (data.success) {
            console.log(
              "✅ CLIENT LAYOUT: Role assigned successfully, reloading user"
            );
            ...
            console.error(
              "❌ CLIENT LAYOUT: Failed to assign role:",
              data.error
            );
            router.push("/");
          }
